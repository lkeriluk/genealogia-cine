...
function calculateRepIndex(f) {
  ...
  const decCounts = decades.map(d => f.sample.filter(m => Math.floor(m.year / 10) * 10 === d).length);
  const total = f.sample.length;
  const expected = total / decades.length;
  const variance = decCounts.reduce((s, c) => s + Math.abs(c - expected), 0) / (total || 1);
  return +Math.max(0, Math.min(1, 1 - variance / 2)).toFixed(2);
}
...
function calcMet(film, diffs) {
  ...
  const normalized = diffs.map(d => {
    ...
    const maxForDir = val >= 0 ? (d.posMax || 3) : (d.negMax || 3);
    return Math.abs(val) / maxForDir;
